...
## 2026-08-26（洛阳钼业 603993.SH 公告采集：akshare cninfo 通道 + adapter 适配）

- **驱动**：洛阳钼业新增后公告簿为空（除财报披露日 NOTICE_DATE），按 §3.6「公告与新闻」来源矩阵补 A 股公告。tdx wenda_notice_query 走交互式 MCP、不便脚本化；kimi stock_finance_data_get_stock_announcement 自 8/13 持续 EMPTY_DATA（handoff §1 已知）；akshare 新增 `stock_zh_a_disclosure_report_cninfo`（巨潮资讯官方披露，列与 tdx wenda_notice_query 一致 title/time/url/source/summary/code/setcode/name），作为本次补采通道。
- **采集**：调 `ak.stock_zh_a_disclosure_report_cninfo(symbol='603993', market='沪深京', start='20250826', end='20260826')` → **135 条** 落盘 `data/raw/akshare/announcement/2026-08-26/run_lyc/603993.SH.csv`（含 _meta.json）。
- **adapter 适配**：
  - `scripts/adapters/akshare.py::parse_announcement_csv`：薄壳，复用 `tdx.parse_announcement_csv`，传 `source='akshare'` 保证 dedup event_id 与不同源采集时隔离。
  - `scripts/adapters/tdx.py::parse_announcement_csv`：新增可选 `source: str = SOURCE` 参数，函数内 SOURCE 引用全部参数化（保持向后兼容，旧调用方无感）。
  - `scripts/pipeline/ingest.py::_ROUTES`：注册 `('akshare', 'announcement')` 路由。
- **入库**：`uv run python -m scripts.pipeline.ingest data/raw/akshare/announcement/2026-08-26/run_lyc` → inserted=135 / errors=0。events.source='akshare'（首次入库 source 字段误标 'tdx'——已清理 raw_objects 与 events 后用真 akshare 路由重导入，确认 135 条均 source='akshare'）。
- **测试**：`tests/test_adapters_akshare.py` 新增 `test_parse_announcement_csv_source_akshare`（验证 source 标签 + 幂等），`uv run pytest -q` **390 全绿**（389+1）。
- **commit**：`1341243 feat: akshare cninfo 公告采集通道`（scripts/adapters/{akshare,tdx}.py + ingest.py + tests）。
- **遗留**：① 历史公告（2007-2025 年）：cninfo 接口默认 1 年窗口，扩窗口需分页多次调，本次仅覆盖 2025-08~2026-08；要补全 2007 年至今历史公告可写分页脚本（按 start/end 滑动）；② 公告内容（PDF/HTML 正文）未下载——表 events 只存 title/summary/url，正文按 §3.6 "PDF、HTML 和新闻正文均作为不可信内容处理" 留二期 LLM 评价时按需拉取；③ 同样通道可补全 watchlist 全池 17 只历史公告（一次按 ticker 循环即可，~30 分钟）。

...
- （2026-09-05 补记⑳）**模拟盘（信号决策力实验）落地**：设计 `specs/2026-09-05-paper-trading-design.md` v2（外部评审 conditional approve 后全采纳）。migration 0011 两表（paper_decisions 一点一决 append-only / paper_positions 状态机）+ `scripts/paper/`（common 枚举/decide/settle/stats）。核心：信号触发点人工录 follow/skip/counter（收盘价系统取、T+1 窗口超窗 late、append-only 冲正、快照冻结、一股一仓结构性 skip 打标）；对称退出（falsification 确认日/stopped_out/deep_exit 档位深度脱离 5%/timeout 60 日停牌顺延）；**判断力 = 主观组合 − 机械基线**（同决策点集全跟的朴素机械化，不依赖 backtest 包）。CLI：`scripts.paper.decide --pending/--pick N --decision`、`settle run/manual-close/reversal`、`stats`；全池日报"模拟盘"段 + 单股 §3 提示行。**与 executions 完全隔离**。测试 +20 = 559 全绿。已知限制：上线首日基线含全部历史点差值无解释力、结构性 skip 占比与 timeout 占比首月回看。详见执行日志 2026-09-05 续②。
- （2026-09-04 补记⑲）**自算筹码分布落地（chip_v1_close_tri）**：设计 `specs/2026-09-04-chip-distribution-design.md` v2（外部 Agent 评审 conditional approve 后修订）。migration 0010 + `scripts/indicators/chip_distribution.py`（换手率衰减模型：A=0.7/close 峰三角核/burn_in=90，复权域直算折回输出）；**不进信号链/daily 默认链**（§2.5 观察项）；现金分红在前复权口径平移历史成本不还原真实成本（设计 §2.3 声明）。CLI：`python -m scripts.indicators.chip_distribution <sym|--all>`；adjust.py 因子重建后 notes 会提示重算。全池首算 25558 行/34 只（25558=bars 数，含 09-04 顺带入库 bars）。测试 537 全绿。**提醒**：09-04 盘后 daily 未跑（indicators/signals 截止 09-03）；展示层接入按需另做。详见执行日志 2026-09-04 续③。

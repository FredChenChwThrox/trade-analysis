...
def _plugin_dir() -> str:
    """kimi-datasource 插件目录：环境变量 KIMI_DATASOURCE_DIR 指定，未设置时报错退出。"""
    plugin_dir = os.environ.get("KIMI_DATASOURCE_DIR")
    if not plugin_dir:
        print("环境变量 KIMI_DATASOURCE_DIR 未设置（kimi-datasource 插件安装目录）", file=sys.stderr)
        raise SystemExit(2)
    return plugin_dir
